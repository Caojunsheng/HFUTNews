...
def insertData(date, title, content, url):
    sql = """INSERT INTO medianews(date, title, content, url)
        VALUES (%s, %s, %s, %s)"""
    print '准备插入数据'
    try:
        # 执行sql语句
...
def getURL(nurl):
    ...
    for info in re.findall(findDiv, html):
        ...
        try:
            text = getContent(url)
            insertData(text['date'], text['title'], text['newsContent'], url)
...
# 接收输入类别、起始页数、终止页数
def main():
    url = 'http://news.hfut.edu.cn/list-6-'
